...
if ( ! function_exists('unsetRequiredSessions'))
{
	function unsetRequiredSessions($sessionVars)
	{
		$instance = get_instance();
		$countVars = count($sessionVars);
		for($i=0;$i<$countVars;$i++)
		{
			$instance->session->unset_userdata($sessionVars[$i]);
		}
		return;
	}
}
if ( ! function_exists('__is_user_logged'))
{
	function __is_user_logged()
	{
		$instance = get_instance();
		if(!empty($instance->session->userdata('ubd_id')))
		{
			return;
		}
		else
...
if ( ! function_exists('__is_manager_logged'))
{
	function __is_manager_logged()
	{
		$instance = get_instance();
		if($instance->session->userdata('ubd_role_id')==1)
		{
			return;
		}
		else
...
if ( ! function_exists('__is_receiptionist_logged'))
{
	function __is_receiptionist_logged()
	{
		$instance = get_instance();
		if($instance->session->userdata('ubd_role_id')==2)
		{
			return;
		}
		else
...
if ( ! function_exists('getBookingTypes'))
{
	function getBookingTypes()
	{
		$instance = get_instance();
		$selectOne = 'bt_id,bt_name';
		$whereOne = 'bt_status=?';
		$paramsOne = [1];
		$dbResultOne = $instance->Booking_Types->SelectWhere($selectOne,$whereOne,$paramsOne);
		return $dbResultOne;
	}
}
if ( ! function_exists('getRoomTypes'))
{
	...
	{
		$instance = get_instance();
		$selectOne = 'rt_id,rt_name';
		$whereOne = 'rt_status=?';
		$paramsOne = [1];
		$dbResultOne = $instance->Room_Types->SelectWhere($selectOne,$whereOne,$paramsOne);
		return $dbResultOne;
	}
}
?>